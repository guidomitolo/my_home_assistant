...
This project features a **bespoke, hand-made Custom Template API** for Home Assistant. Unlike standard integrations, this custom-built API connection is specifically optimized to bridge Home Assistant's data structures with LLMs, enabling AI models to query device states, analyze historical data, and execute actions across your smart home with high precision.
...
* **Historical Analysis:** Access state history to track patterns like energy consumption or security events.
...
| `trigger_service(entity_id, command)` | Executes a command (like `turn_on` or `toggle`) on a specific entity. |
...
**User:** *"Is there anything currently turned on in the Guest Room?"*
* **AI Action:** Calls `search_entities(description="active devices", area="Guest Room")`.
* **Alternative:** Calls `get_states_by_condition(condition="on")` and filters by the Guest Room area.
* **Response:** *"Yes, the 'Guest Bedside Lamp' is currently on."*
